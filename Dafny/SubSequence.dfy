...
// predicate to check if two sequences are equal or not
...
//ensures !b <==> index < 0 && ((index + sub.Length >= str.Length) || str.Length < sub.Length) //&& forall i :: (0 <= i < str.Length - sub.Length) ==> !equal(str[i..(i+sub.Length)], sub[..])
